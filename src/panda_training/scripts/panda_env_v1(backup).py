...
class CobotEnv(gym.Env):
    def __init__(self):
      ...
      self.cmd_handle = Cmd_Handle(exe_time=0.3, mode='cartesian_space')
      ...
      # Action contains end-effector position
      self.action_space = spaces.Box(low=np.array([-1]*6, dtype=np.float32), high=np.array([1]*6, dtype=np.float32))
      ...
      self.goal = np.array([0.35, 0.0, 0.2, 3.14, 0.0, 0.0]) # [0.5, 0.4, 0.3]
    ...
    def step(self, action):
      ...
      action_cmd = current_state[:6] + np.multiply(self.d_p_limit, action)
      action_cmd[3:5] = [3.14, 0]
      ...
      #   state_is_valid = self.collision_handle.cmd_checking(self.move_group.get_current_joint_values()[2:])
      # else:
