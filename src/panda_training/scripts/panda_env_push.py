...
# The action space is the end-effector's x, y and yaw (3)
# The observation space is the end-effector's x, y and yaw and object's x, y (5)
...
from handle_cmd import Cmd_Handle
from handle_collision import Collision_Handle
...
reg = register(
    ...
    entry_point='panda_env_push:CobotEnv',
    max_episode_steps=100,
    )
...
class CobotEnv(gym.Env):
    def __init__(self):
      ...
      self.cmd_handle = Cmd_Handle(exe_time=2.0, mode='cartesian_space')
      ...
      self.p_min = np.array([0.0, -0.5, 0.12, -3.14, -3.14, -3.14]) 	# 140 x 207 x 63 mm
      self.p_max = np.array([0.855, 0.5, 1.19, 3.14, 3.14, 3.14])
      # self.p_min = np.array([0.0, -0.5, -3.14]) 	# 140 x 207 x 63 mm
      # self.p_max = np.array([0.855, 0.5, 3.14])

      # self.p_min = np.array([0.2, -0.855, 0.25, -3.14, -3.14, -3.14]) 	# 140 x 207 x 63 mm
      ...
      # self.d_p_limit = self.time_fraction * np.array([1.7000, 1.7000, 1.7000]) # x, y, z, yaw

      self.p_obj_min = np.array([-1.0, -1.0])
      self.p_obj_max = np.array([1.0, 1.0])
      ...
      # Observation contains end-effector's x, y and yaw and object's x, y
      # 3 + 2 = 5
      self.observation_space = spaces.Box(low=np.concatenate((self.p_min[[0,1,5]], self.p_obj_min)), 
                                          high=np.concatenate((self.p_max[[0,1,5]], self.p_obj_max)), 
                                          dtype=np.float64)

      # Init pose
      self.init_state = np.array([0.3, 0.0, 0.4, 3.14, 0.0, 0.0])

      # Gazebo interface
      self.gazebo = GazeboConnection() 

      # Set goal obj position
      self.goal = np.array([0.4, 0.4]) # [0.5, 0.4, 0.3]
    ...
    def reset(self):
      ...
      # Set goal model
      self.obj_handle.move_static_model('goal', self.goal)

      obj_point = observation[3:]
      ee_point = observation[:2]
      self.last_dis_og = self.distance(obj_point, self.goal)
      self.last_dis_eo = self.distance(obj_point, ee_point)

      # Initial obj pose
      self.obj_handle.object_state_init()
    ...
    def step(self, action):
      ...
      # Constrain
      success = self.cmd_handle.push(action)
      if success != True:
        self.init_pose()
      ...
      # else:
      #   self.cmd_handle.direct_panda_cmd(action_cmd)
    ...
    def take_observation(self):
      ...
      while observed_data is None:
        try:
          ...
          data_obj_pose_local = self.obj_handle.get_obj_pose('pushed_object')
          # print("Get here!")
          # print(data_obj_pose_local)

          # 6 (current ee position) + 6 (goal ee position) = 12
          observed_data = np.array([data_ees_position_local.x, data_ees_position_local.y] + 
                                    [data_ees_rpy_local[2]] +
                                   [data_obj_pose_local.position.x, data_obj_pose_local.position.y]
                                  )
    ...
    def get_reward(self, observed_data):
      reward = 0
      achieve = False
      obj_point = observed_data[3:]
      ee_point = observed_data[:2]

      dis_og = self.distance(obj_point, self.goal)
      ...
      if self.last_dis_og > dis_og:
        reward_og = (self.last_dis_og - dis_og) * self.distance_rew_factor
        print("Reward for obj getting nearer is " + str(reward_og))
      else:
        reward_og = (self.last_dis_og - dis_og) * self.distance_rew_factor 
        print("Reward for obj getting further is " + str(reward_og))

      

      self.last_dis_og = dis_og
      ...
      # if obj_point[0] > 0.7 or obj_point[0] < 0.2 or abs(obj_point[1]) > 0.6:
      #   reward -= 20
      #   achieve = True 

      if self.distance(obj_point, [0,0]) > 0.75 or self.distance(obj_point, [0,0]) < 0.2:
        reward -= 20
        achieve = True 
