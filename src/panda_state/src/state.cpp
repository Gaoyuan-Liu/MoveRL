...
void publishPose(ros::Publisher robot_state_pub, geometry_msgs::PoseStamped pose){
	...
	//msg.x = pose.pose.position.x;
	//msg.y = pose.pose.position.y;
	//msg.z = pose.pose.position.z;
	msg = pose;
...
int main(int argc, char **argv)
{
    ...
    robot_state_pub  = node_handle.advertise<geometry_msgs::PoseStamped>("/panda/robot_states",50);
    // Create a rate
    ros::Rate rate(100);
